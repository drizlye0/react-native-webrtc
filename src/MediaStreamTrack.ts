...
export default class MediaStreamTrack extends EventTarget<MediaStreamTrackEventMap> {
  ...
  /**
   * Private API for setting the zoom level on a video track.
   *
   * @param zoomLevel the zoom level to set (1.0 = no zoom, > 1.0 = zoom in)
   */
  _setZoom(zoomLevel: number) {
    if (this.remote) {
      throw new Error("Not implemented for remote tracks");
    }
    ...
    WebRTCModule.mediaStreamTrackSetZoom(this.id, zoomLevel);
  }
